...
impl ConnectionStream {
    ...
    if self.rd_error.is_some() || self.rd_proto_error.is_some() {
      ...
      loop {
        match read_tls(&mut self.tcp, &mut self.tls) {
          ...
          Err(err) if err.kind() == ErrorKind::WouldBlock => match self.tcp.poll_read_ready(cx) {
    ...
    if self.wr_error.is_some() {
      ...
      loop {
        ...
        match write_tls(&mut self.tcp, &mut self.tls) {
          ...
          Err(err) if err.kind() == ErrorKind::WouldBlock => match self.tcp.poll_write_ready(cx) {
            Poll::Pending => break StreamProgress::RegisteredWaker,
            Poll::Ready(Err(err)) => {
  ...
  fn poll_read(&mut self, cx: &mut Context<'_>, buf: &mut ReadBuf<'_>) -> Poll<io::Result<usize>> {
    loop {
      ...
      match read {
        ...
        StreamProgress::MadeProgress | StreamProgress::NoInterest | StreamProgress::Error => {
...
mod tests {
  ...
  async fn expect_read_1_err(mut conn: &mut ConnectionStream, error: ErrorKind) {
    let mut buf = [0; 1];
    let mut read_buf = ReadBuf::new(&mut buf);
